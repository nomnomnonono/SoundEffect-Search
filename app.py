...
with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                ratio = gr.Slider(
                    ...
                    maximum=1,
                    value=1,
                    label="1 means 'use only text'. 0 means 'use only audio'.",
                )
                topk = gr.Dropdown([5, 10, 20, 30, 40, 50], value="20", label="Top K")
    ...
    mic_button.click(
        search.search,
        inputs=[mic_text_input, mic_audio_input, mic_ratio, mic_topk],
        outputs=mic_output,
    )
